...
class TransferController extends Controller
{
    ...
    public function transfer(Request $request, Form $form = null)
    {
        ...
        foreach ($local_profiles as $local_profile) {
            ...
            $query_where = " where email like ?  OR cell_phone = ?";
            $params = [
                strtolower($local_profile->email),
    ...
    private function transferProfile($connection, $localProfile)
    {
        ...
        try {
            if ($connection->table("profiles")->select('id')->where('email', strtolower($profile_params['email']))->orWhere('cell_phone', 'like', $profile_params['cell_phone'])->first()) {
                return false;
            }
            $resp = $connection->table("profiles")->insert($profile_params);
            if ($resp) {
                $external_profile_id = $connection->table("profiles")->select('id')->where('email', strtolower($profile_params['email']))->first();
            }
        } catch (\Exception $e) {
